...
static int GetHumidity(uint16_t conn_handle, uint16_t attr_handle, struct ble_gatt_access_ctxt *ctxt, void *arg) {
    int humid = 21500;
    int rc = os_mbuf_append(ctxt->om, &humid, sizeof(humid));
   return rc == 0 ? 0 : BLE_ATT_ERR_INSUFFICIENT_RES;
    // return BLE_ATT_ERR_INSUFFICIENT_RES;
}
...
bool getEvent(float *humidity, float *temp) {
    uint32_t t = millis();

    // read the data and store it!
    uint8_t cmd[3] = {0xAC , 0x33, 0};
    // tu warunki sprawdzające status, potrzebne????????????????
    WriteToTheSensor(cmd, 3);

    while (getStatus() & AHTX0_STATUS_BUSY) { // Status bit for busy??
        WaitMs(80);
    }
    ...
    uint32_t tdata = data[3] & 0x0F; // 0xFF?
    tdata <<= 8;
    tdata |= data[4];
    tdata <<= 8;
    tdata |= data[5];
...
uint8_t getStatus(void) {
  uint8_t ret;
  // if (!i2c_dev->read(&ret, 1)  potrzebne?
  if (!i2c_master_read_from_device(I2C_PORT_NUMBER, I2C_AHT20_ADDRESS, ret, 1, I2C_TIMEOUT);) {
    return 0xFF;
  }
  return ret;
}
